...
class robot:
    ...
    def move(self, motion, tolerance = 0.001): # Do not change the name of this function
        ...
        if motion[0] > max_steering_angle:
            raise ValueError, 'Robot can\'t turn backwards'
        if motion[1] < 0.0:
        ...
        steering = random.gauss(float(motion[0]), self.steering_noise)
        dist = random.gauss(float(motion[1]), self.distance_noise)

        beta = ((dist/self.length) * tan(steering)) #beta = turn

        if (abs(beta) > tolerance):
            radius = dist/beta
            cx = self.x - (sin(self.orientation) * radius)
            cy = self.y + (cos(self.orientation) * radius)
            result.orientation = self.orientation + float(beta)
            result.orientation %= 2 * pi
            result.x = cx + (sin(self.orientation + beta) * radius)
            result.y = cy - (cos(self.orientation + beta) * radius)
        else:
            result.x = self.x + (cos(self.orientation) * dist)
            result.y = self.y + (sin(self.orientation) * dist)
            result.orientation = self.orientation + float(beta)
            result.orientation %= 2 * pi
...
#            delta_x = self.x - landmark[0]
#            delta_y = self.y - landmark[1]
            delta_y = landmark[0] - self.y
            delta_x = landmark[1] - self.x

            angle = atan2(delta_y, delta_x) - self.orientation
            if noise:
